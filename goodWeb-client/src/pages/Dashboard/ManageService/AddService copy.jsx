...
const AddService = () => {
  ...
  const addOfferField = (index) => {
    const updatedServices = [...formData.services_offered];
    updatedServices[index].service_offered_des.push('');
    setFormData({
      ...formData,
      services_offered: updatedServices,
    });
  };
  const removeOfferField = (index, offerIndex) => {
    const updatedServices = [...formData.services_offered];
    updatedServices[index].service_offered_des.splice(offerIndex, 1);
    setFormData({
      ...formData,
      services_offered: updatedServices,
    });
                ...
                <div key={index} className="mb-2 row">
                  <div className='col-sm-12 col-md-6'>
                    ...
                    <input
                      type="text"
                      className="form-control"
                      id={`service_offered_title_${index}`}
                  ...
                  <div className='col-sm-12 col-md-6'>
                    {service.service_offered_des?.map((offer, offerIndex) => (
                      <div key={offerIndex}>
                        <label htmlFor={`service_offered_des_${offerIndex}`} className="form-label">
                          Service Description
                        </label>
                        <textarea type="text" className="form-control" value={offer}
                        id={`service_offered_des_${offerIndex}`}
                        {...register(`services_offered[${index}].service_offered_des[${offerIndex}]`)}
                        onChange={(e) => setFormData({ ...formData,
                          services_offered: [
                            ...formData.services_offered.slice(0, index), { 
                              ...service, service_offered_des: [
                                ...service.service_offered_des.slice(0, offerIndex),
                                e.target.value,
                                ...service.service_offered_des.slice(offerIndex + 1),
                              ],
                            },
                            ...formData.services_offered.slice(index + 1),
                          ],
                        })}/>
                        <button type="button" className="btn btn-warning my-1" onClick={() => removeOfferField(index, offerIndex)}>
                          Remove offer
                        </button>
                      </div>
                    ))}
                    <button type="button" className="btn btn-info" onClick={() => addOfferField(index)}>
                      Add offer
                    </button>
                  </div>
                  <div>
